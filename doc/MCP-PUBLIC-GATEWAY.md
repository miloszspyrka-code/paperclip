...
Token telemetry is projected verbatim from persisted run data (`usage_json` / `result_json`) onto both endpoints; nothing is estimated or derived. `paperclipGetRunMetrics` exposes `inputTokens`, `cachedInputTokens`, `outputTokens`, `reasoningTokens`, `totalTokens`, `usageMeasurement`, `provider`, and `model`: missing buckets stay `null`, `reasoningTokens` surfaces only when the runtime persisted it as its own value, and `totalTokens` sums exclusively disjoint persisted buckets (output tokens are excluded from the sum when no separate reasoning bucket exists). These token fields are independent of `supportedObservability`, which keeps meaning structured tool telemetry only. `paperclipListIssueRuns` heartbeat summaries gain a compact `usage` block (`inputTokens`, `cachedInputTokens`, `outputTokens`, `totalTokens`) plus `provider` and `model`; all are `null` when nothing was persisted.
