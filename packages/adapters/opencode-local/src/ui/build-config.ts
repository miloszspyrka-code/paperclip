...
export function buildOpenCodeLocalConfig(v: CreateConfigValues): Record<string, unknown> {
  ...
  const runtimePlugins = (v as unknown as Record<string, unknown>).opencodeRuntimePlugins;
  if (typeof runtimePlugins === "string" && runtimePlugins.trim()) {
    ac.opencodeRuntimePlugins = parseCommaArgs(runtimePlugins);
  }
  return ac;
}
