...
describe("decideWatchdogTick — RTB no-meta-issue + bounded lifecycle (KOMAA-166 A/B, acceptance #2/#3)", () => {
  ...
  it("3 consecutive suspicious ticks for the same source => 0 meta-issues, 0 model, no duplicates", () => {
    ...
    for (let tick = 0; tick < 3; tick += 1) {
      ...
      expect(decision.invokeModel).toBe(false);
      expect(decision.createMetaIssue).toBe(false);
      // idempotency guard stays false; we never open a meta-issue
      expect(shouldOpenNewMetaIssue(openMetaIssueExists)).toBe(false);
      openMetaIssueExists = openMetaIssueExists; // simulation: no new issue created
    }
  });
...
describe("shouldOpenNewMetaIssue — idempotent meta-issue guard (acceptance #3)", () => {
...
describe("evaluateRunBudget — mechanical model-call budget + compaction (KOMAA-166 C, acceptance #4)", () => {
...
describe("buildCompactContinuationPrompt — bounded continuation context (acceptance #4)", () => {
  const ctx = {
...
describe("deriveStructuredRunMetrics — persisted structured telemetry (KOMAA-166 D, acceptance #5)", () => {
